...
public class Part2
{
    private static readonly IDictionary<string, int> Numbers = new Dictionary<string, int>
    {
        { "one", 1 },
        { "two", 2 },
        { "three", 3 },
    ...
    public void Test1()
    {
        ...
        var sum = SumCalibrationValues(document);
    ...
    {
        return document.Split(Environment.NewLine).Sum(RecoverCalibrationValue);
    }

    public static int RecoverCalibrationValue(string input)
    {
        ...
        for (var x = 0; x < input.Length; x++)
        {
            ...
            {
                var remainingLetters = input.Length - x;
                if (remainingLetters < 3) continue;

                var threeLetterWord = input.Substring(x, 3);
                if (Numbers.TryGetValue(threeLetterWord, out var threeLetterNumber))
                {
                    var digit = char.Parse(threeLetterNumber.ToString());
                    if (firstDigit is null) firstDigit = digit;
                    else secondDigit = digit;
                    continue;
                }

                if (remainingLetters < 4) continue;

                var fourLetterWord = input.Substring(x, 4);
                if (Numbers.TryGetValue(fourLetterWord, out var fourLetterNumber))
                {
                    var digit = char.Parse(fourLetterNumber.ToString());
                    if (firstDigit is null) firstDigit = digit;
                    else secondDigit = digit;
                    continue;
                }

                if (remainingLetters < 5) continue;

                var fiveLetterWord = input.Substring(x, 5);
                if (Numbers.TryGetValue(fiveLetterWord, out var fiveLetterNumber))
                {
                    var digit = char.Parse(fiveLetterNumber.ToString());
                    if (firstDigit is null) firstDigit = digit;
                    else secondDigit = digit;
                    continue;
                }
            ...
            if (firstDigit is null) firstDigit = character;
            else secondDigit = character;
        }

        secondDigit ??= firstDigit;

        return int.Parse($"{firstDigit!.Value}{secondDigit!.Value}");
    }
}
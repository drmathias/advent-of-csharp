...
public class Part1
{
    private static (int Red, int Green, int Blue) Possible = (12, 13, 14);
    ...
    public void Test1()
    {
        ...
        var sum = CalculateGameSum(input);
    ...
    public void Test7()
    {
        ...
        Assert.Equal(2476, sum);
    }

    public static int CalculateGameSum(string input)
    {
        return input.Split(Environment.NewLine).Sum(line =>
        {
            var parts = line.Split(':');
            return IsPossible(parts[1]) ? int.Parse(parts[0][5..]) : 0;
        });
    }

    public static bool IsPossible(string line)
    {
        foreach (var selection in line.Split(';').SelectMany(subset => subset.Split(',')))
        {
            var input = selection.Trim().Split(' ');
            var count = int.Parse(input[0]);
            bool isPossible = input[1] switch
            {
                "red" => count <= Possible.Red,
                "green" => count <= Possible.Green,
                "blue" => count <= Possible.Blue,
            };

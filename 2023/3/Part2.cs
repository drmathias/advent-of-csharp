...
public class Part2
{
    ...
    public void Test1()
    {
        ...
        var sum = CalculateSumOfGearRatios(input);
    ...
    public void Test4()
    {
        ...
        Assert.Equal(81463996, sum);
    }

    private static int CalculateSumOfGearRatios(string input) => FindGearRatios(input).Sum();

    private static IEnumerable<int> FindGearRatios(string input)
    {
        var gears = new List<Coordinate>();
        var digits = new Dictionary<Coordinate, char>();
        ...
        for (var y = 0; y < lines.Length; y++)
        {
            ...
            for (var x = 0; x < line.Length; x++)
            {
                ...
                if (character == '*') gears.Add(new Coordinate(x, y));
                else if (char.IsAsciiDigit(character)) digits.Add(new Coordinate(x, y), character);
            }
        }

        var numbers = Schematic.MapDigitsToNumbers(digits);

        foreach (var gear in gears)
        {
            var ratioParts = numbers.Where(number => number.Key.IsAdjacent(gear)).ToArray();
            if (ratioParts.Length == 2)
            {
                yield return ratioParts[0].Value * ratioParts[1].Value;
            }
        }
    }
}
